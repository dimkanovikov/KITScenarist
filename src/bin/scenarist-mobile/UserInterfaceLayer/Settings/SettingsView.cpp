...
{
	m_ui->applicationModuleCharacters->setChecked(_use);
}

void SettingsView::setApplicationModuleLocations(bool _use)
{
	m_ui->applicationModuleLocations->setChecked(_use);
}
...
{
	m_ui->enableAutoReplacing->setChecked(_value);
}
...
{
	if (m_ui->lightTheme->isChecked()) {
		m_ui->colors->setCurrentWidget(m_ui->lightThemeColors);
	} else {
		m_ui->colors->setCurrentWidget(m_ui->darkThemeColors);
	}
}
...
void SettingsView::initConnections()
{
	...
	connect(m_ui->lightTheme, SIGNAL(toggled(bool)), this, SLOT(aboutColorThemeChanged()));
	...
	connect(m_ui->applicationModuleCharacters, &QCheckBox::toggled, this, &SettingsView::applicationModuleCharactersChanged);
	connect(m_ui->applicationModuleLocations, &QCheckBox::toggled, this, &SettingsView::applicationModuleLocationsChanged);
	...
	connect(m_ui->enableAutoReplacing, SIGNAL(toggled(bool)), this, SIGNAL(scenarioEditEnableAutoReplacing(bool)));
	connect(m_ui->spellChecking, SIGNAL(toggled(bool)), this, SIGNAL(scenarioEditSpellCheckChanged(bool)));
