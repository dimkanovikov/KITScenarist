...
namespace UserInterface {
    ...
    public:
        ...
        /**
         * @brief Получить идентификатор элемента идущего перед добавляемым
         * @note Зависит от способа добавления, если новая карточка добавляется через контекстное меню, или двойной клик,
         *       то она вставляется в то место, где был курсор, если добавляется через кнопку в панели инструментов, то
         *       новая карточка будет добавлена после выделенной карточки
         */
        QString beforeNewItemUuid() const;
