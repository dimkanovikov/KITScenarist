...
namespace RtfReader
{
	...
	void TextDocumentRtfOutput::resetCharacterProperties()
	{
		...
		m_textCharFormatStack.top().setFontCapitalization(QFont::MixedCase);
		m_cursor->setCharFormat( m_textCharFormatStack.top() );
	}
	...
	void TextDocumentRtfOutput::setFontUppercase()
	{
		m_textCharFormatStack.top().setFontCapitalization(QFont::AllUppercase);
		m_cursor->setCharFormat(m_textCharFormatStack.top());
	}
