	...
	m_decoder(0)
{
	if (functions.isEmpty()) {
		...
		functions.set("caps", &RtfReader::setTextCapitalization, QFont::AllUppercase);
	...
	if (stylesheet_functions.isEmpty()) {
		...
		stylesheet_functions.set("li", &RtfReader::setBlockLeftIndent);
		...
		stylesheet_functions.set("caps", &RtfReader::setTextCapitalization, QFont::AllUppercase);
...
{
	m_state.block_format.setRightMargin(::pixelsFromTwips(value));
	m_cursor.mergeBlockFormat(m_state.block_format);
}
...
{
	m_state.char_format.setFontCapitalization(QFont::Capitalization(value));
	m_cursor.mergeCharFormat(m_state.char_format);
}
