...
QtZipReader::FileInfo::FileInfo(const FileInfo &other)
{
	operator=(other);
}

QtZipReader::FileInfo& QtZipReader::FileInfo::operator=(const FileInfo &other)
{
...
bool QtZipReader::FileInfo::isValid() const
{
...
public:
	...
	void fillFileInfo(int index, QtZipReader::FileInfo &fileInfo) const;
...
void QtZipPrivate::fillFileInfo(int index, QtZipReader::FileInfo &fileInfo) const
{
...
{
	d->scanFiles();
	QList<QtZipReader::FileInfo> files;
	for (int i = 0; i < d->fileHeaders.size(); ++i) {
		QtZipReader::FileInfo fi;
		d->fillFileInfo(i, fi);
		files.append(fi);
	}
	return files;
...
/*!
	Returns the list of files the archive contains.
*/
QStringList QtZipReader::fileList() const
{
	d->scanFiles();
	QStringList files;
	for (int i = 0; i < d->fileHeaders.size(); ++i) {
...
/*!
	Check if file begins with zip header.
*/
bool QtZipReader::canRead(QIODevice* device)
{
	uchar tmp[4];
	device->peek((char *)tmp, 4);
	return (readUInt(tmp) == 0x04034b50);
}
